...
void Garage::Initialize()
{
	...
	mLineMonitor->setLineThreshold( 1 );
...
void Garage::GoToGarage()
{
	if( mRunManager->getDistanceFromOrigin() <= mGaragePoint )	// 停止地点まで進んでない？
	{
		...
		mRightWheel.setPWM( mForward - mPidController->LeancalControlledVariable(mLineMonitor->getDeviation()) );
		mLeftWheel.setPWM( mForward + mPidController->LeancalControlledVariable(mLineMonitor->getDeviation()) );
	}
	else	// 停止距離まで通過
	{
